...
const request = async <Response> (method: 'GET' | 'POST'| 'PUT'| 'DELETE',
    ...
    options?: CustomOptions | undefined) => {
        ...
        // Nếu không truyền baseUrl hoặc baseUrl = underfined thì lấy giá trị từ envConfig.NEXT_PUBLIC_API_ENDPOINT
        // Nếu truyền baseUrl thì lấy giá trị truyền vào, truyền vào '' đồng nghĩa với việc chúng ta gọi API đến Next.JS Server
        ...
        // Đảm bảo logic bên dưới chỉ chạy ở môi trường client (browser)
        if (typeof window !== 'undefined') {
            if (['auth/login', 'auth/register'].some(item => item === normalizePath(url))) {
                clientSessionToken.value = (payload as LoginResType).data.token
                clientSessionToken.expiresAt = (payload as LoginResType).data.token
